...
def query_agnews(data,model,tokenizer):
    import torch
...
def evaluate_agnews(model,tokenizer,output_path,max_sample=100,wandb_run=None):
    model.eval()
    from tqdm import tqdm
    ...
    for data in tqdm(input_data_lst):
        pred = query_agnews(data,model,tokenizer)
...
if __name__=="__main__":
    ...
    os.makedirs(f"{lora_path}/agnews_steps",exist_ok=True)
    evaluate_agnews(model,tokenizer,f"{lora_path}/agnews_steps/all.json",1000,wandb_run)
