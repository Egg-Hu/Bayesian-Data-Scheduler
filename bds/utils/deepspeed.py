...
from .deepspeed_utils import (
    ...
    get_train_ds_config,
    get_scheduler_train_ds_config, #bak
)
...
class DeepspeedStrategy(ABC):
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        ...
        is_scheduler = isinstance(model, TrainableTensorModule) or  isinstance(model, DataScorerModel)#bak
        assert is_scheduler != is_actor #bak
        ds_config = self.get_ds_train_config(is_actor,is_scheduler) #bak
    ...
    def get_ds_train_config(self, is_actor, is_scheduler=False): #bak
        # DS Config
        ...
        if is_scheduler: #bak
            ds_config = get_scheduler_train_ds_config(
                offload=False,
                adam_offload=self.adam_offload,
                stage=0,
                bf16=False,
...
class MockDeepspeedStrategy(ABC):
    """
    The strategy for training with Accelerator.
    """
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        is_actor = isinstance(model, Actor)
        is_scheduler = isinstance(model, TrainableTensorModule) #bak
        assert is_scheduler != is_actor #bak
        ds_config = self.get_ds_train_config(is_actor,is_scheduler) #bak
